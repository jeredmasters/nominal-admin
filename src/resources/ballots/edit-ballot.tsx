...
export const EditBallotPage = () => {
  const { ballot_id } = useParams();

  if (!ballot_id) {
    return <ErrorPanel text="Must have ballot_id" source="EditBallotPage" />;
  }

  return (
    <TabContainer>
      <TabPanel label="Edit Ballot">
        <EditBallot ballot_id={ballot_id} />
      </TabPanel>
    </TabContainer>
  );
};
interface EditBallot {
  ballot_id: string;
}
export const EditBallot = ({ ballot_id }: EditBallot) => {
  return (
    <SimpleEdit resource={RESOURCE.ballot} id={ballot_id}>
